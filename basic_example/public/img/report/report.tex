...
\begin{figure}[H]
  \centering
	\includegraphics[width=0.8\textwidth,keepaspectratio]{knock.jpg}
...
Each user sends a stream to the MCU, Erizo. Erizo then perform some basic transcoding, if any, before forwarding the data to each other user in the same room.
%Table
\subsubsection{Theory}
Let U denote amount of users in a room, O the amount of users overhearing a certain room and S the bandwidth of a stream. During our measurement a stream started of at 40KB/s but quickly rose to 250KB/s. The data sent to the server is then U*S for each room.

The data sent from the server is U((U-1)*S) + O*U*S. This is where the bottleneck is.

An example. If we have one room with 6 users and 3 people overhearing with a bitrate of 200KB/s results in:

6*200 = 1 200 KB/s sent to the server
6*(5*200) + 3*6*200 = 9 600 KB/s sent from the server.

Let’s assume we have a server with a dedicated 1Gbit connection.

1Gbit = 131 072   KB.
The bandwidth needed to fill a room without overhearing is 6*(5*200) = 6 000 KB/s
131 072/6 000 = 21.8453333333. So a 1Gbit connection can manage almost 22 filled rooms at a time. However, we need to keep in mind that 200 KB/s equals to over 1.5 Mbit/s. In comparison, Skype recommends 500kbps bandwidth for high quality video[25].

As mentioned before, the bottleneck lies in data sent from the server. The CPU usage peaked at a bit over 50% while the video quality of the streams lowered because the server couldn’t send out more than 4 MB/s.
...
The test result showed that the system is really heavy on the bandwidth. We see two solutions to this problem. The first one is to transcode the streams. This would increase the CPU load but drastically decrease the data sent from the server. The second solution is to limit the bitrate of the streams. The video quality as it is now is really high and limiting it to maybe 125KB/s would drastically increase the maximum number of users on the system.
...
We are very happy with the result and we look forward to see any future research in this field.
